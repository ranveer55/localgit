...
class AddEmployee extends Component {
  ...
  componentDidMount() {
    if (this.state.userId) {
      ...
      global.api.getUserAccessLevel(this.state.userId)
        .then(({ accessLevel }) => {
          this.setState({
            userIsNotAdmin: accessLevel != "3",
      ...
      <main className="offset" id="content">
        ...
        <section className="section_box">
          ...
          <form onSubmit={this.form.handleSubmit} className="form_employee js-tabs">
            ...
            {
              this.state.userId && this.state.userIsNotAdmin && this.state.checkedIfUserIsAdmin ? (
                <span
                  className="btn btn-blue btn-sm btn-radius btn-icon-left"
                  style={{ marginBottom: "2rem", marginLeft: "5px", cursor: "pointer" }}
                  onClick={e => {
                    // make use the admin
                    // check if user is admin
                    global.api.makeUserlevel3(this.state.userId)
                      .then(response => {
                        this.setState({
                          userIsNotAdmin: false,
                          checkedIfUserIsAdmin: true

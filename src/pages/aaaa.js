...
load=()=>{
   !this.state.dataLoaded &&this.state.cohorts&&
        ...
        deleteUser = () => {
          ...
          let arrays=[];
          this.state.userId.forEach(element => {
                 arrays.push(element.id);
          });
          console.log(arrays)
          global.api.deleteUserCourse(`${arrays}`)
                .then(data => {
                if (data && data.status) {
                let selectedCohort =undefined;
                const cohorts = this.state.cohorts.map((c) => {
                    console.log("cccccc",c.id)
                  if (c.id === this.state.selectedCohort.id) {
                    console.log("selectedCohort",this.state.selectedCohort.id)
                    console.log("this.state.userId",this.state.userId)
                    const users = this.state.selectedCohort.users.filter((u) => u.userId !==arrays);
